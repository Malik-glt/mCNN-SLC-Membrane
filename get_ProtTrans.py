...
#The data processing procedure involves the utilization of two main folders, each of which contains two additional subfolders labeled as "train" and "test".

def main(input_folder, output_folder):
    os.makedirs(output_folder, exist_ok=True)

    for data_folder in ["Membrane", "Secondary"]:
        ...
        for folder_name in ["train", "test"]:
            ...
            for file_name in os.listdir(input_folder_path):
                ...
                seqs = read_fasta(fasta_file)  # Read the FASTA file
                embeddings = get_embeddings(model, tokenizer, seqs)  # Get the embeddings

                # Save the embeddings to the output file
                np.save(output_file, embeddings["residue_embs"])
